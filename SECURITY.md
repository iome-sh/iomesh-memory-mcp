...
| Shared palace root | **Single-writer contract** — one host process per `PALACE_ROOT`. Cloud: `palace.lock` (pid + start time) fails start if that pid is alive (crash-safety, not flock). Cloud also sets kernel `PalaceConfig.TransactionalIngest` (`wal/pending` intent log, not flock). Local-dev leaves that flag **false** (partial persist). Multi-process writers are unsupported (last-write-wins). Path tenancy is not cloud isolation. |
| Streamable HTTP (`MEMORY_MCP_HTTP_ADDR`) | Network-exposed MCP. Default bind is loopback (`:8080` → `127.0.0.1:8080`). `0.0.0.0` / non-loopback requires `-allow-non-loopback` **and** `MEMORY_MCP_HTTP_SECRET` (fatal before listen if the secret is missing — not a request 401 after bind). When set, the secret (`X-Memory-MCP-Secret` or `Authorization: Bearer`) fail-closes MCP. `GET /healthz` and `GET /ready` stay open (`/healthz` ≠ `/ready`). Unauthenticated HTTP remains residual on loopback when the secret is unset. |
...
- **HTTP unauthenticated on loopback** when the secret is unset — do not expose to untrusted networks. Loopback is the default bind; `0.0.0.0` is refused without `-allow-non-loopback` **and** `MEMORY_MCP_HTTP_SECRET` (fatal before listen). `/healthz` and `/ready` stay open.  
...
- **Path-based tenancy ≠ multi-tenant cloud isolation** — one process, shared code. Omitted and invalid tenant fail closed so list/write stay under `$PALACE_ROOT/<tenant>/` and never share `$PALACE_ROOT/default`. `GET /healthz` and `GET /ready` do not leak tenant or org. Organization isolation for the I/O Mesh broker is a separate HTTP header (`X-IOMesh-Org`) on mesh clients; this host does not implement that.  
- **Single-writer contract** — one host process per palace root. Cloud mode adds `palace.lock` (pid + start time; start fails if that pid is alive) and kernel `PalaceConfig.TransactionalIngest` (`wal/pending`; recover-on-open even when the flag is off). That is crash-safety, not flock. Local-dev leaves TransactionalIngest **false**. Multi-process writers on a shared root remain unsupported. In-process kernel `writeMu` is not flock and not a tenancy boundary.  
...
2. Prefer **stdio** for local TUI attach; if HTTP, keep the loopback default. Non-loopback **requires** `MEMORY_MCP_HTTP_SECRET` (process will not listen without it). `/healthz` and `/ready` stay unauthenticated.  
