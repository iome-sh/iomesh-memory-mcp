...
| Tenant subdirectory (`MEMORY_TENANT` / tool `tenant`) | **Path-based isolation only** — same process residual; not cloud multi-tenant security. |
...
- **Path-based tenancy ≠ multi-tenant cloud isolation** — one process, shared code.  
