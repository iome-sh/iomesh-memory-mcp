...
[`v0.1.1`](https://github.com/iome-sh/iomesh-memory-mcp/releases/tag/v0.1.1).
`@latest` / floating `main` are not production pins. Default `ServerVersion` is
`v0.1.1` (GoReleaser ldflags override on tagged assets). **Not** Memory GA.
...
Server name: **`iomesh-memory-mcp`**. Default version stamp: **`v0.1.1`** (overridden by `make build` / GoReleaser ldflags).
